...
export default function TopicDetail({ topic, category, onBack }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in">
      ...
      <button
        onClick={onBack}
        className="group flex items-center gap-2 text-ink-muted hover:text-ink mb-10 transition-all duration-200 text-sm font-medium"
      >
        <div className="w-8 h-8 rounded-full bg-bg-hover group-hover:bg-ink group-hover:text-white flex items-center justify-center transition-all duration-200">
      ...
      <div className="mb-12 pb-10 border-b border-border/50">
        ...
        <div className="flex items-center gap-2 text-sm text-ink-muted mb-4">
          <span className="w-6 h-6 rounded-lg bg-bg-hover flex items-center justify-center text-xs">{category.icon}</span>
          <span>{category.name}</span>
          <svg className="w-3.5 h-3.5 text-border" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
          </svg>
          <span className="text-ink-secondary font-medium">研究热点</span>
        </div>
        ...
        <div className="flex items-center gap-4 mt-6">
          <div className="flex items-center gap-2 bg-bg-hover rounded-full px-4 py-2 text-sm">
